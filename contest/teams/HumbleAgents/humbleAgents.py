...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if len(mostLikelyGhostPositions) > 0:
      ghostPosDist = [(pos, self.getMazeDistance(oldPos, pos)) for pos in mostLikelyGhostPositions]
      ghostPosDist = sorted(ghostPosDist, key=lambda x: x[1])
      features["ghostDist"] = self.getMazeDistance(myPos, ghostPosDist[0][0])

    if attackerCount != len(self.getOpponents(gameState)):
      foodTuples = [(piece, self.getMazeDistance(oldPos, piece)) for piece in food.asList()]
      foodTuples = sorted(foodTuples, key=lambda x: x[1])
      features["foodDistance"] = self.getMazeDistance(myPos, foodTuples[0][0])
...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if len(pacmen) == 0 and heaviestEater is None:
      ...
      features["closestEnemy"] = min([self.getMazeDistance(pos, myPos) for pos in mostLikelyEnemyPositions])
  ...
  def getWeights(self, gameState, action):
    return {'finalDefense': -100000, 'onDefense': 100000000, "closestEnemy": -30, 'optimalDefenseDistance': -5, 'closestFood': -1, 'closestEater': -15, 'closestFoodWhileTracking': -1}
