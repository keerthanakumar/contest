...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def getPotentialPositions(self, gameState, x, y):
    rows = self.getFood(gameState).height
    cols = self.getFood(gameState).width
    positions = []

    for step in self.steps:
      r = int(x + step[0])
      c = int(y + step[1])
      if r >= 0 and c >= 0 and r < cols and c < rows and not gameState.hasWall(r, c):
  ...
  def closestEnemyIndices(self, gameState, position):
    enemyIndices = []
  ...
  def getMostLikelyPosition(self, enemyIndex):
    bestPos = None
    mostParticles = -1

    for pos in self.enemyParticles[enemyIndex]:
      particles = self.enemyParticles[enemyIndex][pos]
      if particles > mostParticles:
        bestPos = pos

    return pos

  def getMostLikelyEnemyPositions(self):
    positions = []

    for enemyIndex in self.enemyParticles:
      positions.append((enemyIndex, self.getMostLikelyPosition(enemyIndex)))

    return positions

  def getEatenFoodLocations(self, gameState):
    if self.lastFood is not None:
      currentFood = self.getFoodYouAreDefending(gameState).asList()
      eatenFood = []

      for food in self.lastFood.asList():
  ...
  def updateParticles(self, gameState):
    ...
    eatenFood = self.getEatenFoodLocations(gameState)

    chosenIndices = []
    for food in eatenFood:
      indices = self.closestEnemyIndices(gameState, food)
      index = 0
  ...
  def chooseAction(self, gameState):
    ...
    for index in xrange(gameState.getNumAgents()):
      if index in self.enemyParticles.keys():
        dists.append(self.enemyParticles[index])
      else:
        dists.append(None)
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def connectedComponents(self, gameState, points):
    ...
    for pos in self.validPositions:
      ...
      if gameState.hasFood(pos[0], pos[1]):
        self.dfs(gameState, pos, seen)
        components += 1
      else:
        seen.append(pos)
  ...
  def dfs(self, gameState, pos, seen):
    if pos in seen or not gameState.hasFood(pos[0], pos[1]):
      return

    seen.append(pos)
    for nextPos in self.getPotentialPositions(gameState, pos[0], pos[1]):
      seen.append(nextPos)
      self.dfs(gameState, nextPos, seen)
  ...
  def getFeatures(self, gameState, action):
    ...
    #   if dist <= 3:
    #     features["scaredGhostDist"] += dist
    #   else:
    #     features["scaredGhostDist"] -= 1
...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    # positions = self.getMostLikelyEnemyPositions()
    # features["invaderDistance"] = min([self.getMazeDistance(myPos, position) for index, position in positions])
